...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (strncmp(argv[i], "--bit_legnth=", 13) == 0) {
      payload_length = atoi(argv[i] + 13);
    } else {
  ...
  int status = receive(data_buf, desc_buf, comp_buf, wq_info.wq_fd, xfer_size, buf_size, interval_cycle, payload_length);
  if (status == -1){
